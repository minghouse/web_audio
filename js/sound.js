...
function makeFirstOrderFilter(ctx, b0, b1, a0, a1) {
  ...
  biquad1.frequency.value = Fs * 0.027;
  biquad1.Q.value = Q0;
  ...
  biquad2.frequency.value = Fs * 0.027;
  biquad2.Q.value = Q0;
  ...
  biquad3.frequency.value = Fs * 0.027;
  biquad3.Q.value = q;
...
for (var i = 0; i < 8; i++) {
  var a = makeCombFilter(actx, delayT[i], 0.7 + 0.28*0.8, 1*44100);
...
function genFakePiano(){
  var len = 44100 * 2;
  var buff = actx.createBuffer(2, len + 44100 * 108 / 432, 44100);
  var dat = buff.getChannelData(0);
  var k = 432 / 44100 * Math.PI * 2;
  var s = [];
  // var e = 1.2456;
  var e = 2.71828182845904523536/2;
